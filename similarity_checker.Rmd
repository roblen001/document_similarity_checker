...
```{r}
file.list <- list.files(path = 'C:/document_similarity_checker/proposals')
...
```{r}
test <- results %>% filter(proposal_title == 'Estimated transmissibility and impact of SARS-CoV-2')
l <- unlist(test[4])
for (keyword in l) {
  print(keyword)
}
